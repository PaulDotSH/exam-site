...
func writeCodeToFile(data *ProblemData, index int) {
	f, err := os.OpenFile(cppName, os.O_CREATE|os.O_WRONLY, 0755)
	defer f.Close()
	if err != nil {
		fmt.Println(err)
	}
...
//Remove cpp and compiled file, docker will remove them anyway but go complains about having a .cpp file in the same directory as .go files
func clean() {
	os.Remove(cppName)
	os.Remove(compiledName)
}

//Compile the program using g++
func Compile(ctx *context.Context) {
	cmd := exec.CommandContext(*ctx, "g++", cppName, "-o", compiledName)
...
func Run(ctx *context.Context, input string) (string, string, error) {
	cmd := exec.CommandContext(*ctx, fmt.Sprintf("./%v", compiledName))
